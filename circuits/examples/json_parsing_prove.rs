//! Proof generation for verifying the validity of an ECDSA signed Atala identity JSON.
//!
//! This example reads a JWT blob from stdin and generates a proof.
//! If no input is provided, it uses a hardcoded example blob.
...
fn read_stdin() -> io::Result<Vec<u8>> {
    ...
    while buffer.last().map_or(false, |&b| b.is_ascii_whitespace()) {
        buffer.pop();
    }
    Ok(buffer)
}
...
fn main() {
    ...
    eprintln!("Setting up the vk/pk...");
    let vk = compact_std_lib::setup_vk(&srs, &relation);
    let pk = compact_std_lib::setup_pk(&relation, &vk);

    // Read blob from stdin or use default
    let blob = match read_stdin() {
        Ok(input) if !input.is_empty() => input,
        _ => {
            eprintln!("No input from stdin, using default blob");
            BLOB.to_vec()
        }
    };

    eprintln!("Computing instance and witnesses...");
